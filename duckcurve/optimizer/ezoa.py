...
selection, etc.) lives in ``duckcurve.experiments.ezoa_run`` and imports the
``EZOA`` / ``EZOAResult`` symbols from here. Do not put pipeline code in this
file again -- that is what caused the circular import documented in
FIX_NOTES.md.
...
class EZOA:
    ...
    def _accept(self, f_new: np.ndarray, f_old: np.ndarray) -> bool:
        ...
        if dominates(f_old, f_new):
            return False
        # Mutually non-dominated: a real trade-off along the front.
    ...
    def run(self, on_iter: Optional[Callable[[int, EZOAResult], None]] = None) -> EZOAResult:
        # Always (re-)evaluate the current population here rather than trusting
        # self.F from __init__: callers are free to mutate self.X (e.g. seeding
        # a fraction of the population with time_biased_initial_sample) between
        # construction and run(), and stale objective values for those rows
        ...
        for t in range(self.iterations):
            ...
            for i in range(self.population_size):
                # --- Phase 1: foraging, move towards an archive leader ---
                ...
                # --- Phase 2: defense, either flee (S1) or counter-attack (S2) ---
                if self.rng.random() < 0.5:
                    # S1: escape from a lion via a shrinking random walk.
                    # Scaled by the dimension's bound range (not by X[i]
                    # itself), so variables that pass near zero -- e.g. BESS
                    # dispatch -- keep a meaningful step size.
                    step = self.defense_step * (1.0 - progress)
                    defense = self.X[i] + step * (2 * self.rng.random(self.dim) - 1) * self.range
                else:
                    # S2: counter-attack another predator, guided by a second leader
        ...
        # Polish only after the Pareto search, preserving its trajectory.
        self._polish_primary_elite()
        if history_best:
            history_best[-1] = np.min(self.archive.F, axis=0)
