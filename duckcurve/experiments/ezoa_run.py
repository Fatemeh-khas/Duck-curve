"""End-to-end MO-EZOA experiment for the IEEE 33-bus paper case."""
...
class EZOAPipelineResult:
    ...
    best_duck_scenario_result: TimeSeriesResult
    best_duck_objectives: Tuple[float, ...]
    per_seed_results: List[EZOASeedResult] = field(default_factory=list)
        ...
        for unit_a in range(spec.n_bess):
            for unit_b in range(unit_a + 1, spec.n_bess):
                for sign_mode in (-1.0, 1.0):
                    ...
                    # Widely separated hours provide useful SOC-path alternatives
                    # while keeping the derivative-free search computationally small.
                    hour_pairs = sorted(
                        hour_pairs,
                    ...
                    for hour_a, hour_b in hour_pairs:
                        for direction in (step, -step):
                            ...
                            candidate = best_x.copy()
                            candidate[offset:] = proposal.ravel()
                            # Decoding must not rescale either unit; this guarantees
                            # that the aggregate-preserving trial is physically feasible.
...
def _polish_feasible_primary(
    ...
    # Coordinate descent over discrete placements, repeated to convergence.
    for _ in range(3):
    ...
    # Refine each physical BESS independently. This is the only source of
    # inter-unit dispatch/SOC differences; no participation pattern is imposed.
    ...
    # Dispatch changes alter losses and can change the best placement.
    for _ in range(3):
        if not optimize_buses():
            break
...
def run_ezoa_pipeline(
    ...
    ezoa = EZOA(
        ...
        primary_elite_trials=0,  # specialized feasible search runs below
        primary_elite_step=primary_elite_step,
    )

    # Seed part of the population with physically sensible midday charging and
    # evening discharging patterns.  These are only initial guesses; all 24-hour
    # dispatch values remain decision variables optimized by MO-EZOA.
    ...
    # Always preserve one feasible analytical load-leveling candidate.  This
    # directly targets SSS and prevents stochastic runs from missing the flat
    # dispatch pattern that the BESS can physically deliver.
    ezoa.X[0] = _load_leveling_initial_sample(spec, include_losses=include_losses)

    def _on_iter(t: int, result: EZOAResult) -> None:
        if not verbose:
            return
...
def run_ezoa_multiseed_pipeline(
    ...
    **pipeline_kwargs,
    ...
    per_seed_best_sss = []
    per_seed_results: List[EZOASeedResult] = []
    ...
    for k, seed in enumerate(seeds):
        ...
        merged.add(pipe.ezoa.archive.X, pipe.ezoa.archive.F)
        per_seed_best_sss.append(pipe.best_duck_objectives[0])
        per_seed_results.append(
            EZOASeedResult(
                seed=int(seed),
                best_duck_x=pipe.ezoa.archive.X[pipe.best_duck_idx].copy(),
        ...
        # Concatenate convergence histories seed-by-seed purely for the
        # diagnostic convergence plot; each run's optimization is independent.
        history_best.extend(pipe.ezoa.history_best_per_obj)
        history_hv.extend(pipe.ezoa.history_hypervolume)
