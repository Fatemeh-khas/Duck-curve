...
from duckcurve.optimizer import (
...
def resolve_optimizer_config(cfg: dict, optimizer_name: str) -> dict:
    defaults = cfg.get("optimizer_defaults", {}) or {}
    if not isinstance(defaults, dict):
        raise TypeError("optimizer_defaults must be a mapping when provided")

    optimizer_cfg = cfg.get(optimizer_name, {}) or {}
    if not isinstance(optimizer_cfg, dict):
        raise TypeError(f"{optimizer_name} config must be a mapping when provided")

    return {**defaults, **optimizer_cfg}
...
def main(argv=None) -> int:
    ...
    parser.add_argument(
        ...
        help="Optimizer name to dispatch through duckcurve.optimizer.master (default: ezoa)",
    )
    parser.add_argument(
        "--output-dir",
        default=None,
        help=(
            "Where to write figures/CSVs "
            "(default: ./outputs/YYYY-MM-DD_HH-MM-SS next to this script)"
        ),
    )
    parser.add_argument(
    ...
    optimizer_name = normalize_optimizer_name(str(args.optimizer or DEFAULT_OPTIMIZER).strip())
    optimizer_cfg = resolve_optimizer_config(cfg, optimizer_name)
    ...
    configured_seeds = optimizer_cfg.get("seeds")
    if configured_seeds is not None:
        seeds = [int(seed) for seed in configured_seeds]
        if not seeds:
            raise ValueError(f"{optimizer_name}.seeds must contain at least one integer seed")
        if len(seeds) != len(set(seeds)):
            raise ValueError(f"{optimizer_name}.seeds must contain distinct values")
        n_seeds = len(seeds)
        declared_n_seeds = optimizer_cfg.get("n_seeds")
        if declared_n_seeds is not None and int(declared_n_seeds) != n_seeds:
            raise ValueError(
                f"{optimizer_name}.n_seeds must match the number of entries in {optimizer_name}.seeds"
            )
    else:
        n_seeds = int(optimizer_cfg.get("n_seeds", 1))
        if n_seeds < 1:
            raise ValueError(f"{optimizer_name}.n_seeds must be at least 1")
    ...
    archive_capacity = int(
        optimizer_cfg.pop("archive_capacity", optimizer_cfg.pop("archive_size_cap", 100))
    )
    optimizer_cfg.pop("seeds", None)
    optimizer_cfg.pop("n_seeds", None)

    pipe = run_optimizer_multiseed_pipeline(
        optimizer_name,
        spec=spec,
        seeds=seeds,
        population_size=int(optimizer_cfg.pop("population_size", 80)),
        iterations=int(optimizer_cfg.pop("iterations", 180)),
    ...
    optimizer_result = pipe.optimizer_result
    ...
    if bool(cvar_cfg.get("enabled", False)):
        ...
        for archive_index, x in enumerate(optimizer_result.archive.X):
            ...
            cvar_screening_rows.append({
                ...
                "objective_sss": float(optimizer_result.archive.F[archive_index, 0]),
                "objective_daily_loss_kwh": float(optimizer_result.archive.F[archive_index, 1]),
        ...
        pipe.best_duck_objectives = tuple(
            float(v) for v in optimizer_result.archive.F[pipe.best_duck_idx]
        )
        pipe.best_duck_scenario_result = run_timeseries(
            decode(optimizer_result.archive.X[pipe.best_duck_idx], spec),
            include_losses=include_losses,
    ...
    bd_scn, bd_info = decode_with_info(optimizer_result.archive.X[pipe.best_duck_idx], spec)
    knee_scn, knee_info = decode_with_info(optimizer_result.archive.X[pipe.knee_idx], spec)
    ...
    save_fig(plot_pareto_front(optimizer_result.archive.F, highlighted=pipe.knee_idx), out_dir, "fig7_pareto_front", formats)
    save_fig(plot_convergence(optimizer_result.history_hypervolume), out_dir, "fig8_convergence", formats)
    save_fig(
        ...
        [
            {
                ...
                **{
                    ...
                    for j, value in enumerate(optimizer_result.archive.X[i])
                },
            }
            for i, objectives in enumerate(optimizer_result.archive.F)
        ],
    ...
    save_json(
        {
            ...
            "decision_vector": [
                float(value) for value in optimizer_result.archive.X[pipe.best_duck_idx]
            ],
            "objectives": {
